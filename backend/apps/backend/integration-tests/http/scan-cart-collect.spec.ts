...
medusaIntegrationTestRunner({
  testSuite: ({ api, getContainer }) => {
    it('collects: prices weigh line, records sale, decrements WMS, cart PAID', async () => {
      ...
      // Sale total must be a real number (catches BigNumber→NaN coercion): 2×1000 + 1×2500.
      expect(r.data.sale.total_minor).toBe(4500);
      expect((await wms.getStock('v_col', 'va')).on_hand).toBe(8);   // 10 - 2
      expect((await wms.getStock('v_col', 'vw')).on_hand).toBe(9);   // 10 - 1
    });

    it('rejects repricing a fixed-price (non-weigh) line → 400', async () => {
      const cart: any = getContainer().resolve('scanCartService');
      const wms: any = getContainer().resolve('wmsService');
      const c = await cart.createCart({ vendor_id: 'v_col4', staff_id: 's1' });
      const normal = await cart.addLine(c.id, { variant_id: 'va', title: 'A', qty: 1, unit_price_minor: 1000 });
      await wms.incrementStock({ vendor_id: 'v_col4', variant_id: 'va', qty: 5, type: 'restock' });
      await cart.send(c.id);
      const r = await api.post(`/pos/scan-carts/${c.id}/collect`, {
        cashier_id: 'c', terminal_id: 't',
        priced_lines: [{ line_id: normal.id, unit_price_minor: 1 }], // attempt to reprice a fixed line
        paid_amount_minor: 1,
      }).catch((e: any) => e.response);
      expect(r.status).toBe(400);
      // cart must NOT have been collected
      const reloaded = await cart.getWithLines(c.id);
      expect(reloaded.status).toBe('PENDING_CASHIER');
    });
