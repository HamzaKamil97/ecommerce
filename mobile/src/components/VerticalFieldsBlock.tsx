...
export function VerticalFieldsBlock({ data }: Props) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: tokens.fontSize.lg, fontWeight: tokens.fontWeight.semibold, color: tokens.colors.text }}>
      ...
      {rows.map((r, i) => (
        ...
        >
          <Text style={{ fontSize: tokens.fontSize.base, color: tokens.colors.textMuted }}>{r.label}</Text>
          <Text style={{ fontSize: tokens.fontSize.base, color: tokens.colors.text, fontWeight: '600', textAlign: 'right', flex: 1, marginLeft: 12 }}>
            {r.value}
          </Text>
        </View>
      ))}
